...
############################################################################
 {
    my ($name) = @_;
#print "ClcShift for: $name ";    
    my $data = AnalyzeData::StoreArray($name);
#print "found rows: ", int(@{$data}),"\n";
...
############################################################################
sub ClcOpticsMy           #10/22/2007 12:32
############################################################################
 {
    my ($EkspPar) = @_;
    my $dir = $EkspPar->{ClcDir};
    ...
    open OUTDAT, "> $dir$EkspPar->{ExpFile}.das"               or die "Could not open out file $dir$EkspPar->{ExpFile}.das:$!\n";

    my $n = "\x0D\x0A";
    my ($data, $ChShift) = ClcShift("$dir$EkspPar->{ExpFile}.dat");
    for(my $i=0;$i<int(@{$data->[0]});$i++){
        print OUTDAT $data->[0][$i]-$EkspPar->{Shift},"   ",$data->[1][$i]-$ChShift, $n;
    }

    my ($WaveLength, $OutPrefix, $DetectorDat, $NumPnt) = ($EkspPar->{Lambda}, "Temperature", "pindiod.dat", int(@{$data->[0]}));
    $WaveLength = 700, $OutPrefix = "IntegTemp"           if (!ClcGasd::IsNumber($WaveLength));
    $DetectorDat = "dfd1000.dat"                          if ( lc(substr($EkspPar->{Lambda},0,3)) eq "dfd" || $WaveLength>1010 );
    my $Config = "    
    URS_Curve {
        Variables {
	   		File UrsCurve_FileReader {
	     		FileName $dir$EkspPar->{ExpFile}.das DataNames { time s1 } DefaultValue  0
            }
	        Clc  UrsCurve_BrightTempClc {
                VarSignalName  File.s1 OpacityDifCalib2Measure $EkspPar->{Opacity}   WaveLength $WaveLength InterFilterFormName inter
                DetectorEmissivityName $DetectorDat CalibTemp 2700 CalibSignal $Calib
                CalibEmissivityName pirolamp.spl   UseFixedCalibEmissivity -1
                ExtraFiberNorm $EkspPar->{InterfilterIntegral} ExtraSignalNorm $EkspPar->{ExtraSignal}
            }

    	}
        Output  URS_Curve::Output {
	        FileToStore $dir$EkspPar->{ExpFile}.tbr VectorOfNames {
                File.time Clc.$OutPrefix
            }
        }
	    NumIter  $NumPnt
	}
";
    open OUTCFG, "> $dir$EkspPar->{ExpFile}.cfg"               or die "Could not open out file $dir$EkspPar->{ExpFile}.cfg:$!\n";
    print OUTCFG $Config;
    close OUTCFG;
    
    system("urs_curve $dir$EkspPar->{ExpFile}.cfg")             if ($MakeTbr);
}       ##ClcOptics
...
#  Cal         Exp        Lambda  Shift   Opacity
        ( $EkspPar->{CalibFile}, $EkspPar->{ExpFile}, $EkspPar->{Lambda}, $EkspPar->{Shift}, $EkspPar->{Opacity}, $EkspPar->{Calibration}, $EkspPar->{InterfilterIntegral}, $EkspPar->{ExtraSignal} ) = split;

        if (!defined($EkspPar->{ExtraSignal})){
            $EkspPar->{InterfilterIntegral} = 1;
            $EkspPar->{ExtraSignal} = 0;
        }
